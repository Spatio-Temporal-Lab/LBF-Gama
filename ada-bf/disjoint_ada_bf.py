...
def run(c_min, c_max, num_group_min, num_group_max, R_sum, path, model, X_query, y_query, query_urls):
    ...
    Bloom_Filters_opt, thresholds_opt, non_empty_ix_opt = Find_Optimal_Parameters(c_min, c_max, num_group_min,
                                                                                  num_group_max, R_sum, train_negative,
                                                                                  positive_sample)
    fn = 0
    fp = 0
    cnt_ml = 0
    ...
    for i in range(total):
        ...
        if score >= thresholds_opt[-2]:
            ...
            ix = min(np.where(score < thresholds_opt)[0]) - 1
            test_result = 0
            if ix >= non_empty_ix_opt:
                test_result = Bloom_Filters_opt[ix].test(url)
            if test_result == 1 and true_label == 0:
                fp += 1
                cnt_bf += 1
            elif test_result == 0 and true_label == 1:
                fn = fn + 1
