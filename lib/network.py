...
def train(model, train_loader, val_loader, all_memory=634 * 1024,
          all_record=33094122, num_epochs=100, output_acc=True):
...
def validate(model, word_dict, region_dict, dataset):
    ...
    predict_true = 0
    predict_false = 0
    for chunk in pd.read_csv('dataset/' + dataset + '/' + dataset + '.csv', chunksize=chunk_size):
        ...
        csv_filename = 'insert_data_yelp.csv'
...
def create_bloom_filter(dataset, bf_name):
    ...
    data = data['insert']
    print(data.shape[0])
    bloom_filter = BloomFilter(capacity=2091991, error_rate=7.243018718748145e-05)
...
def query(model, word_dict, region_dict, dataset, bloom_filter):
    ...
    for chunk in pd.read_csv('dataset/' + dataset + '/query_data.csv', chunksize=chunk_size):
...
class Bayes_Optimizer:
    def __init__(self, input_dim, output_dim, train_loader, val_loader, all_record, learning_rate=0.005,
                 hidden_units=(8, 512)):
        self.input_dim = input_dim
        self.output_dim = output_dim
    ...
    def target_function(self, num_hidden_units):
        ...
        # 训练模型
        return train(model, train_loader=self.train_loader, val_loader=self.val_loader, num_epochs=10, output_acc=True, all_record=self.all_record)
