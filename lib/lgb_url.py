...
def lgb_validate_url(model, X_train, y_train, train_urls, X_test, y_test, test_urls, threshold):
...
def lgb_validate(model, X_train, y_train, X_test, y_test, threshold):
    prediction_results = model.predict(X_train)
    binary_predictions = (prediction_results > threshold).astype(int)
    misclassified_train = [X_train[i] for i in range(len(X_train)) if binary_predictions[i] == 0 and y_train[i] == 1]
    # indices_train = [i for i in range(len(X_train)) if binary_predictions[i] == 0 and y_train[i] == 1]

    # 对测试集进行预测
    prediction_results = model.predict(X_test)
    binary_predictions = (prediction_results > threshold).astype(int)
    misclassified_test = [X_test[i] for i in range(len(X_test)) if binary_predictions[i] == 0 and y_test[i] == 1]
    # indices_test = [i for i in range(len(X_test)) if binary_predictions[i] == 0 and y_test[i] == 1]

    # 返回合并后的url数组
    return np.concatenate((misclassified_train, misclassified_test), axis=0)
...
def lgb_query_url(model, bloom_filter, X_query, y_query, query_urls, threshold, draw):
    fn = 0
    fp = 0
    cnt_ml = 0
    cnt_bf = 0
...
def lgb_query(model, bloom_filter, X_query, y_query, threshold, draw):
    ...
    for i in range(total):
        true_label = y_query[i]
        # data = X_query.iloc[i].drop(labels={'Label'})
        data = X_query[i, :-1]
        prediction = binary_predictions[i]
        if prediction == 1:
            if true_label == 0:
