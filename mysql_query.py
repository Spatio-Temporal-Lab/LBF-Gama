...
for size in range(64 * 1024, 320 * 1024 + 1, 64 * 1024):
    ...
    total = len(df)
    X = df.drop(columns=['url', 'url_type']).values.astype(np.float32)
    y = df['url_type'].values.astype(np.float32)
    urls = df['url'].tolist()
    ...
    # 筛选出符合条件的 URL
    urls_filtered = [
        url for i, (url, prediction) in enumerate(zip(urls, binary_predictions))
...
query_start_time = time.time()
single_query = "SELECT url FROM urls WHERE url = %s"
cursor.executemany(single_query, [(url,) for url in urls])
